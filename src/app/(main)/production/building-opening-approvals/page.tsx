...
export default async function BuildingOpeningApprovalsPageWrapper() {
  let initialData: BuildingOpeningResponse[] = [];

  try {
    initialData = await serverGetJson<BuildingOpeningResponse[]>('/api/BuildingOpenings/pending-approvals');
  } catch {
    initialData = [];
  }

  return <BuildingOpeningApprovalPage initialData={initialData} mode="approval" />;
}
