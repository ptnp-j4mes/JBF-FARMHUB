...
function resolveHouseSelection(houseGroups, preferredGroupId, preferredHouseCode) {
  ...
  if (targetGroup) {
    const targetHouse = (Array.isArray(targetGroup.houses) ? targetGroup.houses : [])
      .find((house) => house.code === preferredHouseCode) || null;
...
export default function ActivityDailyPage() {
  ...
  const deepLinkContextRef = useRef({
    tab: '', entryDate: '', farmCode: '', groupId: '', houseCode: '',
  });
  const houseSelectionSignatureRef = useRef('');

  // ── Health State ──
  const [healthToggles, setHealthToggles] = useState({
    eating: 'normal', movement: 'normal', breathing: 'normal', skin: 'normal',
  });
  ...
  const applyHouseSelection = useCallback((farmCode, groupId, houseCode) => {
    if (!farmCode) return;
  ...
  const medWarehouseBalances = useMemo(
    () => (Array.isArray(medExecutionContext?.warehouseBalances) ? medExecutionContext.warehouseBalances : []),
    [medExecutionContext],
  );
  ...
  const medSelectedWarehouseLots = useMemo(() => {
    ...
    return medWarehouseBalances.filter(
      (balance) => Number(balance?.warehouseId || 0) === targetWarehouseId && Number(balance?.stockLotId || 0) > 0,
    );
    ...
    () => Array.from(
      new Map(
        (Array.isArray(farmDayPlanSuggestions) ? farmDayPlanSuggestions : [])
          .filter((task) => String(task?.houseCode || '').trim() !== String(selectedHouse || '').trim())
  ...
  const visibleTabs = hasOpenedHouses
    ? TABS
    : TABS.filter((tab) => tab.id === 'overview');

  const tabBadgeCounts = useMemo(() => ({
    mortality: mortalityRecords.length,
    feed: feedRecords.length,
    meds: medRecords.length,
  }), [mortalityRecords.length, feedRecords.length, medRecords.length]);
  ...
  const clearMortalityFormImage = useCallback(() => {
    setMortalityForm((prev) => {
      ...
      return { ...prev, hasImage: false, imageName: '', imageUrl: '', imageStorageUrl: '', imageFileKey: '' };
    });
  }, []);
  ...
  const handleMortalityImagePicked = async (event) => {
    ...
    try {
      ...
      setMortalityForm((prev) => ({
        ...prev, hasImage: true, imageName: file.name || 'image.jpg', imageUrl: previewUrl, imageStorageUrl: '', imageFileKey,
      }));
    } catch { /* ignore */ }
  };
  ...
  const handlePersist = async (submit) => {
    ...
    if (!facilityId) {
      void Swal.fire({ icon: 'warning', title: 'ไม่พบฟาร์มที่ใช้งาน', text: 'กรุณาเลือกฟาร์มก่อนบันทึกข้อมูล' });
      return;
    }
    if (submit) {
      const confirm = await Swal.fire({
        icon: 'question', title: 'ยืนยันส่งรายงาน', text: 'เมื่อส่งแล้วเอกสารจะเป็นสถานะ SUBMITTED',
        showCancelButton: true, confirmButtonText: 'ยืนยัน', cancelButtonText: 'ยกเลิก', reverseButtons: true,
      });
      if (!confirm.isConfirmed) return;
    }
    try {
      setSaving(true);
      const runFromDocNo = extractRun4FromMortalityDocNo(mortalityRecords.find((row) => typeof row.mortalityDocNo === 'string')?.mortalityDocNo);
      const runFromExistingRows = mortalityRecords.map((row) => extractRun4FromMortalityRowId(row.id)).find((value) => Boolean(value)) || '';
      ...
      const uploadedMortalityRecords = await Promise.all(
        mortalityRecords.map(async (record, idx) => {
          ...
          if (!record.imageFileKey) {
            return { ...record, id: rowId, imageUrl: record.hasImage ? uploadService.getMortalityRecordUrl(rowId) : record.imageUrl };
          }
          const pending = mortalityPendingFilesRef.current.get(record.imageFileKey);
          if (!pending?.file) {
            return { ...record, id: rowId, imageUrl: record.hasImage ? uploadService.getMortalityRecordUrl(rowId) : record.imageUrl, imageFileKey: '' };
          }
          ...
          return { ...record, id: rowId, hasImage: true, imageName: uploaded.originalFileName || record.imageName, imageUrl: uploaded.recordUrl, imageStorageUrl: uploaded.url, imageFileKey: '' };
        }),
      );
      ...
      const payload = (() => {
        ...
        if (activeTab === 'mortality') return { ...basePayload, mortalityRecords: uploadedMortalityRecords.map(({ imageFileKey, ...rest }) => rest) };
      ...
      await Swal.fire({ icon: 'success', title: submit ? 'ส่งรายงานสำเร็จ' : 'บันทึกแบบร่างสำเร็จ', timer: 1400, showConfirmButton: false });
    } catch (error) {
      await Swal.fire({
        icon: 'error',
  ...
  const fetchMedicationExecutionContext = useCallback(
    async ({ farmCode, groupId, houseCode, vaccineItemId }) => {
      ...
      const cacheKey = [farmCode.trim(), (groupId || '-').trim() || '-', houseCode.trim(), normalizedVaccineItemId].join('|');
      if (medExecutionCacheRef.current.has(cacheKey)) return medExecutionCacheRef.current.get(cacheKey);
      const context = await vaccinationPlanService.getExecutionContext({
        farmCode, groupId: groupId && groupId !== '-' ? groupId : undefined, houseCode, vaccineItemId: normalizedVaccineItemId,
      });
  ...
  const fetchPendingFeedingPlans = useCallback(
    async ({ date, facilityId, houseId, houseCode }) => {
  ...
  const appendPlanToMedRecords = useCallback(async (planTask) => {
    const planItemId = Number(planTask?.id || 0);
    if (planItemId > 0 && medRecords.some((record) => Number(record.vaccinationPlanItemId) === planItemId)) return;
    ...
    if (vaccineItemId > 0) {
      try {
        executionContext = await fetchMedicationExecutionContext({
          farmCode: selectedFarm, groupId: selectedGroup, houseCode: selectedHouse, vaccineItemId,
        });
      } catch (error) {
    ...
    const mergedBatchAllocations = mergeBatchAllocations(
      executionContext?.batchOptions?.length ? executionContext.batchOptions : (Array.isArray(planTask?.batchAllocations) ? planTask.batchAllocations : []),
    ...
    setMedRecords((prev) => [
      ...prev,
      {
        id: nextRowId(), house: selectedHouse, medName: planTask.vaccineName || '',
        vaccineItemId: vaccineItemId > 0 ? vaccineItemId : '',
        vaccinationPlanItemId: planItemId > 0 ? planItemId : null,
        method: 'ฉีดเข้ากล้ามเนื้อ',
        amount: sumAllocatedHeadcount(mergedBatchAllocations) > 0 ? String(sumAllocatedHeadcount(mergedBatchAllocations)) : Number.isFinite(Number(planTask.headcount)) ? String(planTask.headcount) : '',
        dose: Number.isFinite(Number(planTask.dosesRequired)) ? String(planTask.dosesRequired) : sumAllocatedHeadcount(mergedBatchAllocations) > 0 ? String(sumAllocatedHeadcount(mergedBatchAllocations)) : '',
        warehouseId: defaultWarehouse?.warehouseId ?? '', warehouseName: defaultWarehouse?.warehouseName ?? '',
        stockLotId: Number(defaultWarehouse?.stockLotId || 0) > 0 ? Number(defaultWarehouse.stockLotId) : '',
  ...
  // ── Effects ──
  useEffect(() => {
    const syncMainFacility = () => { setMainFacilityId(getCurrentFacilityId()); setMainFacilityCode(getCurrentFacilityCode() || ''); };
    syncMainFacility();
    window.addEventListener(FACILITY_CHANGED_EVENT, syncMainFacility);
    return () => { window.removeEventListener(FACILITY_CHANGED_EVENT, syncMainFacility); };
  }, []);
  ...
  useEffect(() => {
    ...
    deepLinkContextRef.current = { tab: requestedTab, entryDate: requestedDate, farmCode, groupId, houseCode };
  ...
  useEffect(() => {
    ...
    if (mortalityViewerRef.current) { mortalityViewerRef.current.destroy(); mortalityViewerRef.current = null; }
    mortalityViewerRef.current = new Viewer(mortalityGalleryRef.current, {
      navbar: false, title: true,
      toolbar: { zoomIn: true, zoomOut: true, oneToOne: true, reset: true, prev: true, play: false, next: true, rotateLeft: true, rotateRight: true, flipHorizontal: true, flipVertical: true },
    });
    return () => { if (mortalityViewerRef.current) { mortalityViewerRef.current.destroy(); mortalityViewerRef.current = null; } };
  }, [mortalityRecords]);

  useEffect(() => { return () => { clearPendingMortalityFiles(); }; }, [clearPendingMortalityFiles]);
  ...
  useEffect(() => {
    ...
    const fallbackFacilitiesFromScope = () => {
      ...
      return nodes.map((node) => ({ id: node.facilityNodeId, code: node.facilityCode, name: node.facilityName }));
    };
    const loadOptions = async () => {
      setHydrating(true);
      try {
        try {
          const vaccineResponse = await httpClient.get('/api/OperationsHealth/vaccination-plans/options');
          const vaccines = Array.isArray(vaccineResponse?.data?.vaccines) ? vaccineResponse.data.vaccines : [];
          if (vaccines.length > 0) setMedItemOptions(vaccines.map((item) => ({ id: item.id, name: item.name })));
          else setMedItemOptions(STATIC_MED_OPTIONS);
        } catch { setMedItemOptions(STATIC_MED_OPTIONS); }

        try {
          const feedingOptionsResponse = await httpClient.get('/api/Feeding/options');
          const feedItems = Array.isArray(feedingOptionsResponse?.data?.feedItems) ? feedingOptionsResponse.data.feedItems : [];
          ...
          setFarmWarehouseOptions(farmWarehouses.map((w) => ({ warehouseId: w.id, warehouseName: w.name })));
        } catch (e) {
        ...
        const dashboardResponse = await masterApi.getFarmInformationDashboard(mainFacilityId ?? undefined);
        if (!active) return;
        ...
        const facilities = Array.isArray(dashboardResponse?.facilities)
          ? dashboardResponse.facilities.map((item) => ({ id: item.id, code: item.code, name: item.name }))
          : [];
        ...
        const targetFacilityId = dashboardResponse?.selectedFacilityId
          ?? mainFacilityId
          ?? facilities[0]?.id
          ?? null;
        const selectedFacility = targetFacilityId
          ? (facilities.find((facility) => facility.id === targetFacilityId) ?? facilities[0] ?? null)
          : facilities[0] ?? null;
          ...
          dashboardHouses.map(async (house) => {
            try {
              ...
              return {
                ...
                mortalityRatePercent: detailSummary.mortalityRatePercent ?? house.mortalityRatePercent ?? null,
        ...
        if (selectedFacilityCode) {
          ...
          for (const house of enrichedHouseCards) {
            ...
            if (!grouped[selectedFacilityCode][zoneCode]) {
              grouped[selectedFacilityCode][zoneCode] = { id: zoneCode, label: zoneLabel, houses: [] };
            }
        ...
        for (const facility of facilities) {
          nextHouseMap[facility.code] = sortHouseGroups(Object.values(grouped[facility.code] ?? {}));
        }
        ...
        setFarmOptions([]); setHouseGroupMap({}); setSelectedFarm(''); setSelectedGroup(''); setSelectedHouse('');
        setHydrating(false);
        return;
      }
    };
  ...
  useEffect(() => {
    ...
    if (!preferredFarmCode || preferredFarmCode === selectedFarm || !farmOptions.some((facility) => facility.code === preferredFarmCode)) return;
    setSelectedGroup('');
    setSelectedHouse('');
  ...
  useEffect(() => {
    ...
    const signature = `${selectedFarm}::${groups
      .map((group) => `${group.id}:${Array.isArray(group?.houses) ? group.houses.map((house) => house.code).join(',') : ''}`)
      .join('|')}`;
    ...
    if (hasDeepLinkForCurrentFarm) {
      ...
      deepLinkContextRef.current = { tab: '', entryDate: '', farmCode: '', groupId: '', houseCode: '' };
    } else if (storedSelection) {
      nextSelection = resolveHouseSelection(groups, storedSelection.groupId, storedSelection.houseCode);
    } else {
  ...
  useEffect(() => { resetEntryForm(); setEntryDate(getTodayIsoDate()); }, [selectedFacility?.id]);
  useEffect(() => { setEntryIdsByTab({}); setEntryId(null); }, [entryDate]);
  useEffect(() => { setMortalityForm((prev) => ({ ...prev, deathDay: autoDeathDay })); }, [autoDeathDay]);
  useEffect(() => { setEntryId(entryIdsByTab[activeTab] ?? null); }, [activeTab, entryIdsByTab]);

  // Load vaccination plans
  useEffect(() => {
    let active = true;
    const loadPlanSuggestions = async () => {
      if (!selectedFarm || !entryDate) { setPlanSuggestions([]); setFarmDayPlanSuggestions([]); return; }
      setPlanLoading(true);
      try {
        ...
        const selectedDay = days.find((day) => normalizePotentialBuddhistIsoDate(day?.isoDate) === normalizedEntryDate);
        const rawTasks = Array.isArray(selectedDay?.tasks) ? selectedDay.tasks : [];
        const tasks = rawTasks.filter((t) => { const status = (t?.status || '').toLowerCase(); return status !== 'completed' && status !== 'cancelled'; });
        const sameFarmTasks = tasks.filter((task) => (task?.farmCode || '').trim() === selectedFarm);
        const filtered = tasks.filter((task) => {
          ...
          if (!selectedGroup || selectedGroup === '-' || !taskGroup || taskGroup === '-') return true;
        ...
        if (!active) return;
        setFarmDayPlanSuggestions([]); setPlanSuggestions([]);
      } finally {
  ...
  useEffect(() => {
    ...
    if (!selectedFarm || !selectedHouse || vaccineItemId <= 0) { setMedExecutionContext(null); setMedExecutionLoading(false); return; }
    setMedExecutionLoading(true);
    fetchMedicationExecutionContext({ farmCode: selectedFarm, groupId: selectedGroup, houseCode: selectedHouse, vaccineItemId })
      .then((context) => {
        if (!active) return;
        ...
        setMedForm((prev) => {
          ...
          const nextBatchAllocations = mergeBatchAllocations(context.batchOptions, prev.batchAllocations);
          const nextHeadcount = sumAllocatedHeadcount(nextBatchAllocations);
          const nextWarehouseId = Number(prev.warehouseId || 0) > 0 ? Number(prev.warehouseId) : Number(context.warehouseBalances?.[0]?.warehouseId || 0);
          const candidateLots = (context.warehouseBalances ?? []).filter((balance) => Number(balance?.warehouseId || 0) === nextWarehouseId && Number(balance?.stockLotId || 0) > 0);
          const hasCurrentLot = candidateLots.some((balance) => Number(balance?.stockLotId || 0) === Number(prev.stockLotId || 0));
          return {
            ...prev,
            ...
            stockLotId: hasCurrentLot ? prev.stockLotId : Number(candidateLots[0]?.stockLotId || 0) > 0 ? Number(candidateLots[0].stockLotId) : '',
          };
        });
      })
      .catch((error) => { console.error('Failed to load vaccination execution context for daily meds.', error); if (active) setMedExecutionContext(null); })
      .finally(() => { if (active) setMedExecutionLoading(false); });
    return () => { active = false; };
  }, [fetchMedicationExecutionContext, medForm.vaccineItemId, selectedFarm, selectedGroup, selectedHouse]);
  ...
  const handleMortalityImageClick = useCallback((row) => {
    const index = mortalityRecords.filter((item) => getMortalityImageSrc(item)).findIndex((item) => item.id === row.id);
  ...
  return (
    <Box sx={{ p: { xs: 1.5, md: 2 }, pb: { xs: 10, md: 2 }, bgcolor: UI.bg }}>
      ...
      {/* Tabs */}
      <Box sx={{ borderRadius: 3.2, border: `1px solid ${UI.border}`, bgcolor: UI.panel, boxShadow: UI.shadowSoft, p: { xs: 0.85, md: 1 }, bgcolor: UI.panelSoft, mb: 1.5 }}>
        <Tabs
          value={activeTab}
        ...
        >
          {visibleTabs.map((tab) => (
            <Tab
              key={tab.id}
              value={tab.id}
              icon={React.cloneElement(tab.icon, { sx: { fontSize: 22 } })}
              iconPosition="start"
              label={
                tabBadgeCounts[tab.id] > 0
                  ? `${tab.label} (${tabBadgeCounts[tab.id]})`
                  : tab.label
              }
            />
          ))}
      ...
      <Box>
        ...
        {activeTab === 'health' && (
          <HealthTab
            ...
            onToggleChange={(key, value) => setHealthToggles((prev) => ({ ...prev, [key]: value }))}
            selectedHouse={selectedHouse}
          />
        )}
        {activeTab === 'mortality' && (
      ...
      <Box ref={mortalityGalleryRef} sx={{ position: 'fixed', width: 1, height: 1, overflow: 'hidden', opacity: 0, pointerEvents: 'none', left: -99999, top: -99999 }}>
        {mortalityRecords.filter((row) => getMortalityImageSrc(row)).map((row) => (
          <img key={`gallery-${row.id}`} src={getMortalityImageSrc(row)} alt={row.imageName || 'mortality-evidence'} />
        ))}
      </Box>

      {/* Action Buttons */}
      <Box sx={{ borderRadius: 3.2, border: `1px solid ${UI.border}`, bgcolor: UI.panel, boxShadow: UI.shadowSoft, p: { xs: 1.4, md: 1.8 }, mt: 2 }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} justifyContent="center" alignItems="center">
          <Button
            variant="outlined"
            disabled={saving}
            onClick={() => handlePersist(false)}
            sx={{ width: { xs: '100%', sm: '30%' }, minWidth: 180, borderRadius: 2, boxShadow: UI.shadowSoft, bgcolor: '#fff' }}
          >
            บันทึกแบบร่าง
          </Button>
          <Button
            ...
            sx={{ width: { xs: '100%', sm: '30%' }, minWidth: 180, borderRadius: 2, boxShadow: UI.shadowSoft, bgcolor: UI.accent, '&:hover': { bgcolor: '#10473f' } }}
          >
            ส่งรายงาน
          </Button>
        </Stack>
