...
interface StockAdjustmentRequestFiltersProps {
  searchTerm: string;
  statusFilter: string;
  dateFrom: string;
  dateTo: string;
...
export function StockAdjustmentRequestFilters({
  searchTerm,
  statusFilter,
  dateFrom,
  dateTo,
  ...
  onDateToChange,
  onClear,
}: StockAdjustmentRequestFiltersProps) {
  return (
    <Box
      sx={{
        display: 'grid',
    ...
    >
      <TextField
        label="ค้นหา"
        placeholder="เลขที่เอกสาร, Facility"
        size="small"
        value={searchTerm}
